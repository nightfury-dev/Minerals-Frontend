...
class InputLab extends Component {
    ...
    getInputLabData = async () => {
        ...
        const excellData = res.data.inputLabs.map(data => {
            return {
                ...
                lot_number: data.charge.map(ch => moment(ch.date).format(`${res.data.settings.date_format} HH:mm:ss`)),
                stock_sample: data.charge.length > 0 ? (data.sample_type.stockSample === false ? res.data.inputLabs.filter(d => d.sample_type.stockSample === true)
                    ...
                    ).map(i => i.sample_type.sampleType + " " + i.material.material + " " + i.client.name + " " + moment(i.charge[0].date).format(`${res.data.settings.date_format} HH:mm:ss`)) : ''
                ) : '',
        ...
        this.setState({
            ...
            date_format: res.data.settings.date_format
        })
    }
    ...
    handleClickMButton = (item) => {
        ...
        if (_isStock) {
            if (item.material_left === 0 && item.charge.length === 0) {
                ...
                const data = {
                    value: item._id,
                    label: item.material.material + " " + item.client.name + " " + moment(item.charge[0].date).format(`${this.state.date_format} HH:mm:ss`) + " " + item.material_left
                }
                this.setState({ stock_data: [data] })
            }
        } else {
            ...
            filtered_stocks.map(s => {
                data.push({
                    value: s._id,
                    label: s.material.material + " " + s.client.name + " " + moment(s.charge[0].date).format(`${this.state.date_format} HH:mm:ss`) + " " + s.material_left,
    ...
    getCertificateData = async (row) => {
        const inputIds = [this.state.selectedId]
        this.state.allData.filter(d => d.sample_type.stockSample === true && d.material._id === this.state.allData.filter(d => d._id === this.state.selectedId)[0].material._id)
            ...
            .map(data => {
                inputIds.push(data._id)
            })
        const columns = []
        const pdf_data = []
        await Promise.all(inputIds.map(async (id) => {
            const selectedRow = this.state.allData.filter(d => d._id === id)[0]
            const pdfData = {}
                        ...
                        row.sample_type.stockSample === false &&
                            (
                                this.state.allData.filter(data => data.sample_type.stockSample === true)
                                    ...
                                    .map(i => (
                                        stockSamples.push(i.sample_type.sampleType + " " + i.material.material + " " + i.client.name + " " + moment(i.charge[0].date).format(`${this.state.date_format} HH:mm:ss`))
                                    ))
                            )
                                                ...
                                                <ul>
                                                    {
                                                        this.state.allData.filter(data => data.sample_type.stockSample === true)
                                                            ...
                                                            .map((i, index) => (
                                                                <li key={index}>
                                                                    {i.sample_type.sampleType + " " + i.material.material + " " + i.client.name + " " + moment(i.charge[0].date).format(`${this.state.date_format} HH:mm:ss`)}
                                                                </li>
                                                            ))
                                                    }
                                                </ul>
